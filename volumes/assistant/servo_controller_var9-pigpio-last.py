...
class ServoController:
    def __init__(self, config_path='config.yaml'):
        if not PIGPIO_AVAILABLE:
            raise ImportError("pigpio не установлен")
        ...
        # Подключение к daemon на хосте (через network_mode: host)
        self.pi = pigpio.pi()
        
        if self.pi is None:
            raise RuntimeError("Не удалось подключиться к pigpio daemon — запустите sudo pigpiod на хосте")
        
        self.pi.set_mode(self.pin, 1)  # 1 = PWM
        self.pi.set_PWM_frequency(self.pin, 50)
        self.pi.set_PWM_range(self.pin, 1000)
        
        logger.info(f"✅ Сервопривод SG-90 инициализирован на GPIO{self.pin} (pigpio daemon)")
